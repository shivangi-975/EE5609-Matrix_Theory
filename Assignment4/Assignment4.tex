...
\end{align}
In equilateral triangle all angles are equal.Hence from equation $\ref{my22}$ \\
\begin{align}\label{kk}
\angle{ABC} =\angle{BCA} =\angle{BAC} =60^{\circ}
\end{align}
Hence  from ${\ref{kk}}$,we proved that all angles of equilateral triangles are ${60^{\circ}}$each
\end{document}
